...
public class ItemSpawner : MonoBehaviour
{
    ...
    public GameObject ItemSpawn (int n, Vector3 _pos, Vector3 _rota)
    {
        STATE state = (STATE)Enum.ToObject (typeof (STATE), n);
        ...
        switch (state)
        {
            case STATE.KEY:
                Debug.Log ("Key Spawend");
                return Instantiate (keyPrefab, _pos, Quaternion.Euler (_rota));
            case STATE.NOISY:
                Debug.Log ("Noisy Spawend");
                return Instantiate (noisyPrefab, _pos, Quaternion.Euler (_rota));
            case STATE.GARBAGE:
                Debug.Log ("Garbage Spawend");
                return Instantiate (garbagePrefab, _pos, Quaternion.Euler (_rota));
            default:
                break;
        }
